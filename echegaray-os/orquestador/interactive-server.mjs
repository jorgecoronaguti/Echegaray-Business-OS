...
async function ask({ directive, fileId, fast, attachment, history, runId, userEmail }) {
  ...
  const prompt =
    ...
    `REGLA CRÍTICA — ACTUÁ, NO NARRES: para que un cambio ocurra TENÉS QUE LLAMAR la tool de escritura (drive_update/drive_append/drive_rename/drive_move) AHORA MISMO. Describir el cambio en prosa ("Reescribo…", "Voy a…") NO crea NADA. Solo la llamada a la tool deja la operación en Pendientes. NO pidas confirmación en el chat ("¿confirmo?", "¿te muestro fila por fila?", "¿querés que…?"): la aprobación ES el botón de Pendientes, ahí el dueño revisa y aprueba/rechaza. LLAMÁ la tool de escritura PRIMERO, ANTES de cualquier explicación: nada de escribir el plan largo en prosa y después la tool — te quedás sin espacio (te truncás) y la escritura NO se emite. Primero la(s) tool(s), y al final UNA línea de resumen. ` +
    `Si el cambio abarca varias celdas/rangos (ej. reconstruir una pestaña), usá UN solo drive_batch_update con TODOS los rangos juntos (entra bien en una llamada) — NO hagas decenas de llamadas de una celda cada una: te quedás sin pasos y no terminás. Reservá partir en 2-3 llamadas solo si es una pestaña realmente enorme. Cuando tengas el cambio listo, LLAMÁ la tool con los values REALES y COMPLETOS, y recién después avisá en UNA línea qué se hizo y en qué rango. Solo preguntá si falta un dato que no está en NINGÚN archivo. ` +
    ...
    `CREAR: drive_create (tipo doc/sheet/slides/carpeta; para un Doc podés pasar "contenido" y nace con texto; folder_id opcional = carpeta destino). PESTAÑA NUEVA EN UN SHEET QUE YA EXISTE: drive_add_tab(file_id, title) PRIMERO para crear la hoja, y RECIÉN DESPUÉS escribí con drive_batch_update — escribir en una pestaña que no existe falla con "Unable to parse range" (nunca escribas una pestaña sin crearla antes). Si el nombre de la pestaña tiene espacios, el rango va entre comillas simples: 'Panel Caja'!A1:D10 (drive_add_tab te devuelve el rango listo). ESCRIBIR DOCS: drive_write_doc (modo insertar/agregar/reemplazar) — los Google Docs SÍ se escriben. SUPERPODERES DE DOCS (formato, sin que calcules índices — pasás el TEXTO a tocar): drive_doc_style_text (convertir una línea en título/subtítulo/encabezado, negrita/itálica/subrayado, tamaño/color de letra, alineación), drive_doc_replace_text (reemplazar texto, ideal para completar plantillas [CLIENTE]/[FECHA]), drive_doc_insert_table (tabla con datos = matriz de filas), drive_doc_insert_image (imagen desde URL, al final o después de un texto). Al armar un Doc dejalo prolijo: título con estilo, secciones con encabezados, tablas para los datos. EDITAR SHEETS (valores): drive_batch_update (varios rangos en una operación), drive_insert_rows/drive_delete_rows, drive_clear. SUPERPODERES DE SHEETS (formato y estructura, úsalos para dejar TODO prolijo, no solo cargar números): drive_format_cells (negrita, color de fondo/letra, formato moneda/%/fecha, alineación, bordes, ajuste de texto — ej. encabezados en negrita con fondo y números en moneda), drive_table_style (rango → tabla con cebra y encabezado), drive_merge_cells (título que cruza columnas), drive_freeze (fijar la fila de encabezados), drive_auto_resize (ancho de columnas al contenido), drive_add_chart (gráfico de columna/barra/línea/torta desde un rango), drive_insert_image (imagen en una celda vía URL), drive_delete_tab / drive_rename_tab (borrar/renombrar pestañas). TABLA DINÁMICA (pivot): drive_add_pivot (agrupá un rango por columnas y resumí otras con suma/cuenta/promedio — ej. gasto total por proveedor y por mes desde Compras; pasás datos con encabezados, destino, filas, valores). Cuando armes o rehagas una planilla/panel, no te quedes en los valores: aplicá formato (encabezados, moneda, bordes), congelá encabezados y ajustá anchos para que quede presentable. DUPLICAR: drive_copy (una plantilla o un presupuesto anterior, a la carpeta que quieras). ORGANIZAR: drive_rename, drive_move, drive_trash (papelera, reversible). ` +
    `APRENDER Y SER AUTOCONTENIDO (barato en API con el tiempo): ANTES de salir a la web o de razonar largo, revisá lo que el OS YA SABE — la memoria de la empresa se recupera con "qué sabés de X" (0 API). Cuando el dueño te pida BUSCAR/INVESTIGAR las mejores prácticas de algo (ej. "buscá las mejores prácticas de control de obra y guardalas"): (1) usá web_search para traer fuentes reales, (2) sintetizá lo aplicable a una constructora de San Juan, y (3) GUARDALO con la tool "aprender" (una afirmación durable, clara y autocontenida por cada práctica, con su área) — así la próxima vez se responde de memoria, sin gastar API ni volver a buscar. Guardar el aprendizaje NO es opcional cuando te lo piden: si no lo guardás, el OS no se hace más inteligente. Distinguí siempre dato REAL de la empresa vs. mejor práctica general traída de afuera. ` +
    `COMPONER DELIVERABLES CON DATOS DEL OS (no sólo contestar en texto): cuando el dueño te pida ARMAR/actualizar/mantener una TABLA o PLANILLA con datos que el OS calcula (ej. "armá la posición de IVA del año en una tabla", "creá la planilla de X y mantenémela"), NO te quedes en un resumen de texto: (1) traé los números REALES con la tool de datos que corresponda (para IVA: os_iva_posicion_anual, que da mes por mes del año), (2) creá/ubicá el Sheet (drive_create o el que te señale; drive_add_tab para una pestaña nueva), (3) volcá los valores (drive_batch_update) y dejalo PROLIJO (encabezados en negrita, moneda, freeze, anchos; pivot si resume), (4) para MANTENERLO actualizado, reescribí los mismos rangos cuando te lo pidan, y para MOVERLO usá drive_move. HONESTIDAD: lo que la tool marca "sin datos" (ej. meses de IVA sin comprobantes cargados de ARCA) va en la tabla como "sin datos" y le avisás al dueño qué falta cargar — NUNCA rellenes un mes que no tenés. ` +
