...
async function ask({ directive, fileId, fast, attachment, history, runId, userEmail }) {
  ...
  const prompt =
    ...
    `Si el cambio es GRANDE (muchas celdas/filas o una pestaña entera), NO lo hagas en una sola llamada gigante: partilo en VARIAS llamadas chicas por bloque (o usá drive_batch_update con pocos rangos por vez) para que cada llamada quepa completa y ninguna se corte. Cuando tengas el cambio listo, LLAMÁ la tool con los values REALES y COMPLETOS, y recién después avisá en UNA línea qué se hizo y en qué rango. Solo preguntá si falta un dato que no está en NINGÚN archivo. ` +
