...
async function driveRegistry(attachment, userEmail) {
  ...
  const registry = { ...driveReadTools(google), ...driveWriteTools(google), ...sheetsFormatTools(op ? google : null), ...docsFormatTools(op ? google : null), ...osDataTools(), ...webSearchTools(), ...learnTools(), ...obraTools(), ...workspaceTools({ google: op ? google : null }), ...appsheetPedidosTools({ google: op ? google : null }), ...gastoSheetTools(op ? google : null), ...sheetRenderTools(op ? google : null) }
...
async function ask({ directive, fileId, fast, attachment, history, runId, userEmail }) {
  ...
  const guiaInvestigar = researchLearnIntent
    ? ` INVESTIGAR Y GUARDAR: revisá primero lo que el OS YA SABE ("qué sabés de X", 0 API). Si te piden buscar mejores prácticas: usá web_search, sintetizá lo aplicable a una constructora de San Juan, y GUARDALO con la tool "aprender" (una afirmación durable por práctica, con su área). Guardar no es opcional: si no lo guardás, el OS no se hace más inteligente.`
    : ''
