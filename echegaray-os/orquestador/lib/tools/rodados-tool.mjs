  ...
  return {
    'rodados.presupuestos': {
      ...
      schema: {
        ...
        description:
          'PRESUPUESTOS DE RODADOS — los presupuestos que el dueño juntó para comprar una camioneta de obra, con unidad, precio, gastos, total, vigencia (dice si ya venció), formas de pago propuestas y ficha técnica (carga, potencia, seguridad de serie vs opcional), más la condición del crédito asociado. Usalo cuando el dueño pregunte "¿qué presupuestos tengo para rodados?", "¿cuánto sale la camioneta?", "¿qué opciones de pago me dieron?", "¿el presupuesto sigue vigente?". Cada dato dice de qué adjunto salió, y lo que no se pudo leer se informa en "falta" — nunca se completa con un valor razonable. Sin parámetros.',
      ...
      async run() {
        try {
          ...
          return {
            ...
            falta: loQueFalta(),
            nota:
              'La TNA 0% del crédito es NOMINAL: en un crédito UVA el costo es el ajuste del capital por CER, que el presupuesto no declara. No afirmar que el crédito es gratis ni compararlo contra el descubierto como si costara cero.',
          }
        } catch (e) {
