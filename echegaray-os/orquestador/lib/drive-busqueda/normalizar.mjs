...
/** Marca interna para apartar la ñ mientras se quitan los acentos. Es un carácter de
 *  control: no puede aparecer en el nombre de un archivo de Drive. */
const MARCA_ENIE = '\u0001'
...
export function plano(s) {
  return String(s ?? '')
    // LA Ñ SE PROTEGE ANTES DE SACAR ACENTOS. En NFD la ñ se descompone en n + tilde, y esa
    // tilde cae en el mismo rango que la de "tracción": sacarla a ciegas convertía "Diseño"
    // en "diseno" y "Año" en "ano". Es una letra del alfabeto, no un acento.
...
export function singular(t) {
  ...
  // Sólo las consonantes con las que una palabra castellana puede TERMINAR en singular.
  // Sin este recorte, "avances" (avance + s) perdía la 'e' y quedaba "avanc".
  if (/[lrndjszxy]es$/.test(s)) return s.slice(0, -2)       // jornales → jornal
  if (/[^s]s$/.test(s)) return s.slice(0, -1)               // obras → obra, avances → avance
  return s
}
