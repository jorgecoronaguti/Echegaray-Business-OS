...
export const MODELOS = Object.freeze({
  ...
  'vision.epp': {
    capacidad: 'vision',
    modelo: 'Hansung-Cho/yolov8-ppe-detection',
    revision: 'ac0027bd38',
    ejecucion: 'local-cpu',
    proveedor: 'local',
    licencia: 'MIT (pesos) sobre runtime Ultralytics AGPL-3.0',
    estado: ESTADO.BLOQUEADO,
    medido: null,
    porQue: 'BLOQUEADO POR LICENCIA DEL RUNTIME, y es un bloqueo del ecosistema entero, no de un modelo. Revisado el catalogo el 04/09/2026: `keremberke/yolov8*-hard-hat` NO DECLARA licencia (la ausencia no es permiso), `melihuzunoglu/ppe-detection` es AGPL-3.0, y el unico permisivo de verdad —`qualcomm/PPE-Detection`, BSD-3— NO TIENE PESOS en el repo: solo un `release_assets.json` que apunta a su AI Hub y apunta a NPU de Snapdragon. `Hansung-Cho` publica los pesos como MIT pero declara `library_name: ultralytics`, y Ultralytics es AGPL-3.0: correrlo en el servidor que sirve la app obligaria a publicar la app entera bajo AGPL.',
    reingreso: 'DECISION LEGAL, NO TECNICA: exportar los pesos MIT a ONNX usa Ultralytics una vez en tiempo de construccion y el ONNX resultante corre con onnxruntime (MIT). Si eso alcanza para no contaminar la app lo decide un abogado, no este archivo. Alternativa limpia: un detector de personas con licencia permisiva (DETR/RT-DETR) mas una regla de color sobre la region de la cabeza, que no detecta casco pero si «persona sin nada en la cabeza».',
    alternativas: ['qualcomm/PPE-Detection (BSD-3, sin pesos publicados)', 'keremberke/yolov8m-hard-hat-detection (sin licencia declarada)'],
  },
  'reranker.es': {
    capacidad: 'rerank',
