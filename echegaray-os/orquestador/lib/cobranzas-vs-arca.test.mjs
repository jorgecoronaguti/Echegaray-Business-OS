...
test('una B con número que ARCA no tiene se denuncia con su fila', () => {
  const c = conciliarCobranzasConArca([cob({ comprobante: '01-00000048', factura: serial(2026, 1, 6), cliente: 'ARCOR' })], [], { hoy: HOY, cols: COB_HOY })
...
test('la fecha de Cobranzas en OTRO mes que la de ARCA se denuncia — el IVA va al mes de ARCA', () => {
  ...
  const c = conciliarCobranzasConArca([cob({ comprobante: '1-203', factura: serial(2026, 4, 7) })], [arca(1, 203, 2026, 1, 21, 1000)], { hoy: HOY, cols: COB_HOY })
  ...
  const ok = conciliarCobranzasConArca([cob({ comprobante: '1-227', factura: serial(2026, 8, 18) })], [arca(1, 227, 2026, 8, 21, 1000)], { hoy: HOY, cols: COB_HOY })
...
test('una factura de ARCA sin fila se busca por importe entre las B sin número', () => {
  ...
  const c = conciliarCobranzasConArca([cob({ neto: 8375000, cliente: 'MESSINA', factura: serial(2026, 7, 2), cobro: serial(2026, 7, 29) })], [arca(1, 214, 2026, 7, 2, 8375000)], { hoy: HOY, cols: COB_HOY })
...
test('una B sin comprobante con fecha de factura vencida se lista con su fecha de cobro', () => {
  const c = conciliarCobranzasConArca([cob({ factura: serial(2026, 8, 18), cobro: serial(2026, 12, 30), cliente: 'Quattropani' })], [], { hoy: HOY, cols: COB_HOY })
  ...
  const p = conciliarCobranzasConArca([cob({ factura: serial(2026, 10, 9) })], [], { hoy: HOY, cols: COB_HOY })
  assert.equal(p.vencidasSinEmitir.length, 0)
})

test('las N no se concilian, y sin diferencias el informe es vacío', () => {
  const c = conciliarCobranzasConArca([cob({ cat: 'N', factura: serial(2026, 8, 1) })], [], { hoy: HOY, cols: COB_HOY })
