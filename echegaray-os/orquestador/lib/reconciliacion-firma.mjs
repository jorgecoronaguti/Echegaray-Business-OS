 ...
 */
export async function reconciliar(google, deps, fileId, pestana, { ref = pestana, sellarFn = sellarFirma } = {}) {
  ...
  if (!diffs.length) {
    // La firma difería por formato/estructura sin cambio de contenido de celda. Nada que preservar:
  ...
  // ── RESPETAR TUS EDICIONES SIN CONGELAR (27/07) ──
  // El dueño, con razón: "arreglá el bug que hace que no respete mis modificaciones, NO que congeles
  // todo". Congelar la pestaña entera protegía la edición pero también la dejaba sin refrescar nunca —
  // una sobrecorrección. Lo correcto: PRESERVAR exactamente las celdas que tocaste (tu valor gana,
  // siempre) y REGENERAR el resto con datos frescos. Cada celda tuya se registra 'activa'; el choke
  // point la re-inyecta sobre lo que produce el generador, así tu cambio SOBREVIVE a cada regeneración
  // y todo lo demás se mantiene al día.
  //
  // Preservar TODAS las celdas cambiadas (sin clasificarlas) es estrictamente más seguro que el intento
  // anterior que perdió trabajo: no hay forma de que una celda tuya se "reclasifique" y se pierda —
  // todas se preservan. El ÚNICO caso donde re-inyectar por A1 sería incorrecto es un cambio
  // ESTRUCTURAL (insertaste/borraste filas y todo se corrió): ahí no se adivina, se protege y se pregunta.
  const filasNoVacias = (g) => (g || []).filter((f) => (f || []).some((c) => norm(c) !== '')).length
  const filasPrev = filasNoVacias(previo)
  const filasAct = filasNoVacias(actual)
  if (filasPrev !== filasAct) {
    return {
      entendido: false, resuelto: false,
      motivo: `cambió la estructura de "${pestana}" (${filasPrev}→${filasAct} filas con contenido): la dejo bajo tu control, no la toco`,
      aprendidas: [], adoptadas: [],
      preguntas: [{ celda: diffs[0].celda, pregunta: `Cambiaste filas en "${pestana}" (no sólo celdas sueltas). La dejo tal cual hasta que confirmes: ¿la regenero desde cero o mantengo tu versión?` }],
    }
  }

  // Cambio a nivel celda: preservo TODAS tus celdas y devuelvo el resto al generador. Cada celda tuya
  // queda 'activa' → el choke point la re-inyecta en cada regeneración. Reseño la firma al estado actual
  // (tu versión pasa a ser el baseline) y descando: la pestaña vuelve a mantenerse sola, respetándote.
  for (const d of diffs) {
    await registrarCelda(deps, fileId, pestana, d.celda, {
      valorDueno: d.actual, valorOs: d.previo, causa: CAUSAS.OVERRIDE, accion: ACCIONES.ADOPTAR, estado: 'activa',
    }).catch(() => {})
  }
  if (await candadaPorAuto(deps, fileId, pestana)) {
    await sellarFn(google, fileId, pestana, ref).catch(() => {})
  ...
  return {
    entendido: true, resuelto: true,
    motivo: `preservo ${diffs.length} celda(s) tuya(s) y regenero el resto con datos frescos`,
    aprendidas: diffs, adoptadas: diffs, preguntas: [],
  }
}
