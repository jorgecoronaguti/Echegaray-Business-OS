...
//
// POR QUÉ EXISTE (31/07). Dos scripts necesitan la misma pregunta —"¿estos dos textos son el mismo
// movimiento?"— y cada uno tenía su propia respuesta, más floja: el deduplicador comparaba el concepto
// EXACTO (normalizando sólo mayúsculas y espacios) y por eso no veía una familia entera de duplicados; la
// conciliación contra el extracto agregó la comparación por prefijo. Con dos definiciones, el mismo par de
// filas era "duplicado" para uno y "movimientos distintos" para el otro.
//
...
//   2. RECORTADO: la semilla guardó "Pago haberes - 260701507" y el CSV trae el número repetido al final;
//      una captura de pantalla corta el concepto en el ancho de la columna.
...
/** El número de cheque anotado a mano en el concepto ("Cheque debitado - Nº 221" → "221"). */
...
/**
 * Agrupa filas que son EL MISMO movimiento, dentro de un conjunto que ya comparte fecha e importe.
 *
