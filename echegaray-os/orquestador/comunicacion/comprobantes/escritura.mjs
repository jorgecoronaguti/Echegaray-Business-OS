 ...
 */
function avisosDeAlta(altas) {
  const l = []
  for (const a of altas?.altas ?? []) l.push(`Proveedor NUEVO dado de alta: **${a.nombre}** (CUIT ${a.cuit}). Si no es él, avisame.`)
...
function avisosDuros(datos, varios = [], descalces = null) {
  ...
  l.push(...avisosDeAlta(datos?.altas))
...
export function textoCargado(filas, yaEstaban, datos, { pendientes = [], suma = null, varios = [] } = {}) {
  ...
  l.push(...avisosDeAlta(datos?.altas).map((t) => `⚠ ${t}`))
