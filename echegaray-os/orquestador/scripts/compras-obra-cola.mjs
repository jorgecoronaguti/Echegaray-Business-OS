#!/usr/bin/env node
// Arranque del worker que lleva al Sheet la obra elegida en la app para una fila de Compras. La lógica
// vive en `comunicacion/compras/cola-obra.mjs` (probada con dobles); acá sólo se cablean Postgres y
// Google y se reporta.
//
...
function reportarSeco(plan) {
  console.log(`cola de Obra (Compras) EN SECO: ${plan.length} pendiente(s) · nada se escribió`)
  for (const p of plan) {
    ...
    console.log(`  fila ${p.fila} [${p.id}] ${que}`)
  }
}
...
async function main() {
  ...
  // Se reporta SIEMPRE, incluso el cero: un worker que calla es indistinguible de uno roto.
    ...
    console.log(`cola de Obra (Compras): ${c.aplicado} aplicados · ${c.rechazado} rechazados · `
