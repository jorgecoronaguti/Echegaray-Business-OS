...
//        del plan que siguen diciendo exactamente lo que se escribió y estaban vacías en el respaldo.
//
...
async function revertir(rutaPlan) {
  ...
  console.log(`  vaciar ${r.vaciar.length} celda(s) · no son mías ${r.noSonMias.length}`)
  ...
  if (!r.vaciar.length) { console.log('\n✓ nada que revertir.'); return }
  if (!APLICAR) { console.log('\n(ensayo: no vacié nada. Para vaciar: --revertir <plan> --aplicar)'); return }
  ...
  const quedan = r.vaciar.filter((x) => T(despues.get(x.fila)) !== '')
  for (const x of quedan) console.error(`  ✖ ${letra}${x.fila} sigue diciendo «${despues.get(x.fila)}»`)
  if (quedan.length) { console.error('\n✖ la reversa no aterrizó entera.'); process.exit(1) }
  console.log(`\n✓ ${r.vaciar.length} celda(s) vacías, releídas.`)
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const corrida = REVERTIR ? revertir(REVERTIR) : principal()
