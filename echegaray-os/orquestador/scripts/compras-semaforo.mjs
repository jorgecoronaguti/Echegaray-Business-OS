...
import { FILA0, ROTULO, columnasValores, esSemaforoConocido, formulaEstadoPago } from '../lib/compras-valores.mjs'
...
async function main() {
  ...
  // LAS LETRAS SALEN DEL RÓTULO (14/09/2026): con «Obra» insertada en L, el semáforo pasa de Z a AA.
  const COLS = columnasValores((await google.readSheetValues(ID, rangoEncabezado(PESTANA)))?.[0] ?? [])

  // EL RÓTULO ES LA LLAVE. Si la columna dice otra cosa, alguien movió las columnas y esto no es mío.
  const cab = String((await google.readSheetValues(ID, `${PESTANA}!${COLS.semaforo}3`))?.[0]?.[0] ?? '').trim()
  if (cab !== ROTULO.semaforo) {
    throw new Error(`${COLS.semaforo}3 dice "${cab}" y no "${ROTULO.semaforo}". No es mi columna: no la piso.`)
  }

  const rango = `${PESTANA}!${COLS.semaforo}1:${COLS.semaforo}${hoja.rows}`
  ...
  for (let i = 0; i < antes.length; i++) {
    ...
    if (t === formulaEstadoPago(fila, COLS)) alDia++
    else if (!esSemaforoConocido(t, fila, COLS)) ajenas.push({ fila, t })
  }
  const invisibles = antesVisto.filter((f) => glifosInvisibles(String(f?.[0] ?? '')).length).length
  console.log(`${PESTANA}!${COLS.semaforo} · ${conFormula.length} fórmula(s) de la fila ${conFormula[0]} a la `
  ...
  if (ajenas.length) {
    ...
    for (const a of ajenas.slice(0, 10)) console.error(`   ${COLS.semaforo}${a.fila}  ${a.t.slice(0, 110)}`)
    process.exit(1)
  }
  ...
  console.log(`\nREPARTO POR ESTADO (de la columna ${COLS.estado}, que no se toca):`)
  for (const [k, n] of [...repartoAntes].sort((a, b) => b[1] - a[1])) console.log(`  ${String(n).padStart(5)}× ${k}`)
  console.log(`\nla fila ${desde} pasa a:\n  ${formulaEstadoPago(desde, COLS)}`)

  if (!APLICAR) { console.log('\n(sin --aplicar: no escribí nada)'); return }

  const filas = conFormula.map((f) => ({ values: [{ userEnteredValue: { formulaValue: formulaEstadoPago(f, COLS) } }] }))
  const col = idxCol(COLS.semaforo)
  ...
  const malas = conFormula.filter((f) => celda(despues, f - 1) !== formulaEstadoPago(f, COLS))
  ...
  if (ciegas.length) { console.error(`✖ siguen ${ciegas.length} con glifo invisible: ${ciegas.slice(0, 5).map(([f, v]) => `${COLS.semaforo}${f}="${v}"`).join(' ')}`); mal = true }
  if (errores.length) { console.error(`✖ ${errores.length} celda(s) en error: ${errores.slice(0, 5).map(([f, v]) => `${COLS.semaforo}${f}="${v}"`).join(' ')}`); mal = true }
