...
//
//   node orquestador/scripts/xsas-cuadro.mjs quattropani
//   node orquestador/scripts/xsas-cuadro.mjs quattropani --sin-modelo   # 0 API: sólo caché y CAD
//
...
//
// ═══ POR QUÉ NO SE LLAMA «xsas-estado» ═══
//
// Ese nombre ya estaba tomado por otro script, y su invariante es lo contrario de la de éste:
// «describir la inteligencia no puede necesitar la inteligencia» — contesta con el proveedor
// apagado, y ahí es cuando importa que conteste. Éste, en cambio, CORRE el motor sobre un
// proyecto, así que alcanza la puerta al modelo por el grafo de imports. Son dos preguntas
// distintas y no pueden compartir archivo: pisarlo rompía el único control que guarda esa
// separación, y el test lo cazó en la corrida completa.
...
if (!termino) {
  console.error('uso: xsas-cuadro.mjs <termino> [--sin-modelo] [--sin-regiones] [--detalle]')
  process.exit(1)
}
