...
import {
  archivosDeLaQuincena, avisoDeRecibos, filasDeRecibos, totalesDeRecibos, type EstadoDeRecibo, type FilaDeRecibo,
...
import { ALTO_LIQ, Cuadro, Cuerpo, Encabezado, Fila, Hueco, MONO, Total } from './tabla'
...
export async function SolapaRecibos({ quincenaPedida, hoy, parametros = {}, hrefDe }: Partial<PropsDeSolapa> & { hoy: string }) {
  ...
  const aviso = avisoDeRecibos(filas, rotuloQuincena(q))
  const errores = [
  ...
  return (
    <section data-testid="pantalla-recibos">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', paddingBottom: 8 }}>
        <form method="get" data-testid="recibos-quincena" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <select name="quincena" defaultValue={q.desde} aria-label="Quincena"
            style={{ height: 32, padding: '0 8px', borderRadius: 6, border: `1px solid ${V.lineaFuerte}`, fontSize: '12.5px', background: '#FFFFFF' }}>
...
function FilaDeRecibos({ f }: { f: FilaDeRecibo }) {
  ...
  return (
    <Fila columnas={COLS} alto={ALTO_LIQ.filaAngosta} testid={`recibo-${f.personaId}`} celdas={[
      ...
      f.driveFileId
        ? <a key="p" href={`https://drive.google.com/file/d/${f.driveFileId}/view`} target="_blank" rel="noreferrer"
