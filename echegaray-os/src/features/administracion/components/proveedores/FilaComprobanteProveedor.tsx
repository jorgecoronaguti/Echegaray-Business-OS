...
export function FilaComprobanteProveedor({ c, papelesSinLeer, opcionesObra, obraEditable }: {
  c: CompraConPapel
  papelesSinLeer: boolean
  /** Los rótulos elegibles de la columna «Obra». Los mismos que Compras: `getOpcionesDeObra`. */
  opcionesObra: string[]
  /** `false` = la base todavía no publica la columna Obra en `proveedor_compra`: se muestra, no se edita. */
  ...
  return (
    <div data-testid="fila-comprobante-proveedor" style={{ borderBottom: `1px solid ${V.lineaFila}` }}>
      ...
      >
        ...
        {/* ═══ LA COLUMNA OBRA ES EL MISMO CONTROL QUE EN COMPRAS ═══

            `ObraEnLinea` entra por la ÚNICA puerta (`compra_obra_asignar`), que desde la migración
            20260915T2300 también mueve `costos_obra` y `compra_obra_asignada`: elegir acá cambia el costo
            por obra en el acto, y la ficha se revalida al confirmar. El rótulo sale de `obra_celda` /
            `obra_id` + `destino` de la fila —lo que una persona ELIGIÓ— y, si la celda está vacía, de la
            inferencia del sync, marcada como tal. Nunca del texto libre de la J: «Quattropani» o
            «Mamposteria» no identifican una obra; «OB-0008 · NOMBRE» sí.

            La fila es de alto fijo: el texto del papel viaja en el `title` del contenedor, que es donde
            se va a buscar el detalle y no la identidad.

            EL FILO ROJO SIGUE ATADO A `obra_texto` y no al rótulo: si la columna Obra no se pudiera
            leer, atarlo al rótulo pintaría de rojo la ficha entera y anunciaría un desvío que no
            existe. Una guarda que falla cerrada sobre un dato que no pudo leer miente. */}
        <span
          className="flex min-w-0 items-center" data-testid="compra-obra"
          title={c.obra_texto?.trim() ? `El papel dice «${c.obra_texto.trim()}»` : undefined}
        >
          <ObraEnLinea
            fila={c.fila}
            celda={c.obra.celda}
            rotulo={c.obra_rotulo}
            inferida={c.obra.origen === 'inferida'}
            opciones={opcionesObra}
            editable={obraEditable}
            cuerpo="12px"
          />
          {/* LO QUE LA CELDA NO SE PUDO CREER: `obra_inconsistencia` lo escribe el sync. Se muestra, no se corrige. */}
          {c.obra.inconsistencia && (
            <span title={c.obra.inconsistencia} className="flex shrink-0" style={{ color: V.neg }}>
