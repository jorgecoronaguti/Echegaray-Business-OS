...
test('la celda SIN ANÁLISIS no mete adentro la deuda de cómputo', () => {
  ...
  // El defecto que atrapa: `contexto={p.n_sin_computo > 0 ? ... : 'partidas'}`, que hacía leer
  // «3 · 2 sin cómputo» como si el 3 se descompusiera. Son dos deudas distintas.
  assert.ok(src.includes('contexto="partidas"'), 'la quinta celda dice «partidas», como el canon 15')
  assert.equal(src.includes('n_sin_computo > 0'), false, 'la deuda de cómputo vive en su chip, que filtra')
})
