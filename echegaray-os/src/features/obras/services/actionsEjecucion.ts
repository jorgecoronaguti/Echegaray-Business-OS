...
import { filaDeEjecucion, leerParteDiario } from './parteDiario'
...
//
// «NO QUIERO LAYOUT NUEVO» (dueño, 23/09/2026): la 06 manda, por frente en curso, «Producción hoy»
// y «Comentario», y aparte la novedad del día. Todo viaja en UN formulario y se guarda con UNA acción:
//
//   produccion_<actividad> → obra_ejecucion (cantidad → la fracción la deduce `parte_tarea`;
//                            manual → fraccion + declarada = true)
//   comentario_<actividad> → obra_ejecucion.comentario del mismo parte («opcional»)
//   novedad                → obra_actividad_nota del primer frente en curso (`agregarNota`, la misma
//                            acción que el panel de la tarea). La 06 no la ata a una tarea.
//
// LO QUE YA NO VIAJA: personas, activos y horas por persona. La 06 dibuja «Quién vino» de sólo
// lectura (las horas entran por Personal) y no dibuja equipos ni «quién y con qué». El contrato
// anterior (`personas_<id>`, `activos_<id>`, `hh_<persona>`, `novedad_destino`) se retiró.
//
// EL MISMO DÍA SE CORRIGE, NO SE DUPLICA: si ya hay un parte web de esa actividad en esa fecha, se
// actualiza. Volver a guardar no suma dos veces.
// ═══════════════════════════════════════════════════════════════════════════════════════════════
...
export async function guardarParteDiario(obraId: string, form: FormData): Promise<Resultado> {
  ...
  if (leido.renglones.length === 0 && !novedad) {
    return { ok: false, error: 'El parte está vacío: cargá la producción de hoy o una novedad.' }
  }
  ...
  for (const r of leido.renglones) {
    ...
    const fila = { ...filaDeEjecucion(a.metodo_avance, r.produccion), comentario: r.comentario }
    ...
    if (previo) {
      const { error } = await supabase.from('obra_ejecucion').update(fila).eq('id', (previo as { id: string }).id)
  ...
  if (novedad) {
    ...
    if (!actividadId) efectos.push('la novedad NO se guardó: no hay frente en curso al que colgarla')
    else {
      ...
      efectos.push(r.ok ? 'novedad guardada' : `la novedad NO se guardó: ${r.error}`)
    }
  }
