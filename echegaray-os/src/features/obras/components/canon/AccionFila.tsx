...
export function AccionFila<A extends unknown[] = []>({
  accion, args, children, testid, tono = 'neutro', alto = 28,
}: {
    ...
    <form action={ejecutar} style={{ display: 'inline-flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px' }}>
      <button
        ...
        style={{
          ...
          border: `1px solid ${peligro ? C.negBorde : C.borde}`, background: C.superficie,
          color: peligro ? C.neg : C.tintaMedia, cursor: pendiente ? 'default' : 'pointer',
