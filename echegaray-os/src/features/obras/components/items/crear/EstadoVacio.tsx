...
//               tres tarjetas en grilla de 3, `padding:22px 22px 20px`, radio 10, `min-height:190`,
...
export function EstadoVacio({ obraId, presupuesto, lineaBaseSellada, diasHabiles, plazo }: {
  ...
  const opciones = [
    {
      id: 'presupuesto', href: `${base}&crear=presupuesto`, icono: P.base, rotulo: 'convertir_partida_a_plan',
      ...
      bajada: presupuesto ? `${presupuesto.rotulo} · ${presupuesto.nPartidas} partidas` : 'sin presupuesto vinculado',
      accion: 'Empezar', destacada: true,
    },
    {
      id: 'planilla', href: `${base}&crear=planilla`, icono: P.subir, rotulo: 'propuesta · no existe hoy',
      titulo: 'Pegar la planilla',
      texto: 'Las filas del Sheet, con su numeración 1 · 1.1 · 1.1.1, fechas y días teóricos. La ponderación se reparte por días hasta que alguien la cargue.',
      bajada: 'propuesta · no existe hoy', accion: 'Elegir', destacada: false,
    },
    {
      id: 'mano', href: `${base}&crear=mano`, icono: P.editar, rotulo: null,
      titulo: 'Armar a mano',
      texto: 'Un rubro, y adentro lo demás. Enter agrega hermano, Tab baja un nivel.',
      bajada: 'un rubro y adentro lo demás', accion: 'Elegir', destacada: false,
    },
  ]
    ...
    <>
      ...
      <div className="hidden md:flex" data-testid="estado-vacio" style={{ padding: '40px 30px 44px', flexDirection: 'column', gap: '28px', maxWidth: '1120px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <div style={{ fontSize: '13.5px', color: C.tintaSuave }}>Rubro › épica › historia › tarea › subtarea. Tres formas de empezar; se pueden mezclar.</div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          {opciones.map((o) => (
            <Link key={o.id} href={o.href} prefetch={false} data-testid={`crear-${o.id}`} style={{
              display: 'flex', flexDirection: 'column', gap: '12px', padding: '22px 22px 20px', minHeight: '190px', textDecoration: 'none',
              border: `1px solid ${o.destacada ? C.grafito : C.borde}`, borderRadius: '10px', color: C.tinta,
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ color: C.tintaMedia, display: 'flex' }}><Ico d={o.icono} s={20} /></span>
                {o.rotulo && <span style={{ fontSize: '11px', color: C.tenue }}>{o.rotulo}</span>}
              </div>
              <div style={{ fontSize: '15px', fontWeight: 600 }}>{o.titulo}</div>
              <div style={{ fontSize: '12.5px', color: C.tintaSuave, lineHeight: 1.5, flex: 1 }}>{o.texto}</div>
      ...
      <div className="flex md:hidden" data-testid="estado-vacio-telefono" style={{ padding: '16px 16px 100px', flexDirection: 'column', gap: '16px' }}>
        ...
        {opciones.map((o) => (
          <Link key={o.id} href={o.href} prefetch={false} data-testid={`crear-telefono-${o.id}`} style={{
            display: 'flex', alignItems: 'center', gap: '12px', minHeight: '72px', padding: '0 14px', textDecoration: 'none',
            border: `1px solid ${o.destacada ? C.grafito : C.borde}`, borderRadius: '8px', color: C.tinta,
          }}>
