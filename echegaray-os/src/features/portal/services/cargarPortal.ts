...
import {
  getMiObra, getMisCertificados, getMisConsultas, getMisDocumentos, motivoSinObra,
} from './portalService'
...
export async function cargarPortal(obraId?: string): Promise<DatosDelPortal> {
  ...
  // `motivoSinObra` no es un error de lectura: es una parte de la pantalla que esta base todavía
  // no puede contestar (`obra_canonica` no es legible para el rol `cliente`). Va al mismo lugar
  // que los errores porque para el que mira es lo mismo — algo no está y quiere saber por qué.
  const avisos = [...new Set(
    [mi.error, motivoSinObra(mi.data ?? null), certificados.error, documentos.error, consultas.error]
