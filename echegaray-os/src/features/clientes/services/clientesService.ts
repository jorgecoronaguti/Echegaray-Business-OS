...
import {
  armarClientes, armarDocumentosCliente, armarFuentesActividad, armarNotasCliente,
  armarObrasPorCliente, normalizar,
...
export async function getDocumentosCliente(
  ...
  return { data: armarDocumentosCliente(vinculos ?? [], archivos ?? []), error: null }
}
...
export async function getActividadCliente(
  ...
  const archivosDeDrive = await archivosDeDriveDe(
    supabase, (documentos.data ?? []).map((d) => d.drive_file_id as string))

  const fuentes: FuentesActividad = armarFuentesActividad({
    ficha: ficha.data,
...
/** Los certificados de las obras del cliente, CRUDOS. Sin obras no hay consulta que hacer. El
 *  nombre de la obra lo pone `armarFuentesActividad`, que es donde vive ese cruce para los dos
 *  transportes. */
async function certificadosDe(
  ...
  const { data } = await supabase
    ...
    .in('obra_canonica_id', obras.map((o) => o.obra_id as string))
  return (data ?? []) as Record<string, unknown>[]
}

async function archivosDeDriveDe(
