...
test('la ficha no dibuja el avance de obra, y en su pista va el COSTO', () => {
  ...
  // VACÍO NO ES CERO NI «—»: sin permiso o en una cara que no los transporta, las celdas de costo callan.
  // Un «—» afirmaría que este trabajo no gastó nada, y puede llevar $ 154 M. Son CUATRO desde el
  // 18/09/2026: Materiales · Subcontratos · Otros · Mano de obra.
...
test('en el teléfono la tabla de trabajos rueda dentro de su caja, y no empuja la página', () => {
  ...
  const plantillas = plantillasDeObras()
  const bloque = codigoListas().slice(codigoListas().indexOf('const COLS_OBRAS'))
  // 928 = 180 del nombre + 664 de pistas + 84 de gaps, con Subcontratos y Otros (18/09/2026).
