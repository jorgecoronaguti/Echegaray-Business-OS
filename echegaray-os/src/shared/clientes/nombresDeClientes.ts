// id de cliente → nombre para mostrar, leído de `clientes` (la fuente) con el formato único.
...
export async function nombresDeClientes(supabase: SupabaseClient): Promise<Map<string, string>> {
  try {
    const { data, error } = await supabase.from('clientes').select('id, nombre_comercial, razon_social')
