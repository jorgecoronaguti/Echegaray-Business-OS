...
export default async function FlujoCajaPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      {/* Vista principal: CALENDARIO interactivo — cada día muestra sus conceptos */}
      <section className="mt-6">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-sm font-bold text-gray-900 uppercase">Calendario</h2>
          <div className="flex flex-wrap items-center gap-3 text-[11px] text-gray-500">
            <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-green-500" /> cobro</span>
            <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-red-500" /> pago</span>
            <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-orange-500" /> cheque</span>
            <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-purple-500" /> tarjeta</span>
            <span className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded border border-red-200 bg-red-50" /> saldo proy. negativo</span>
          </div>
        </div>
        <CalendarioFlujo dias={cal.dias} hoyIso={new Date().toISOString().slice(0, 10)} />
        {cal.dias.length === 0 && (
