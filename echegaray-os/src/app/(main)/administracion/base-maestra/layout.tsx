...
// TIEMPO REAL (dueño, 15/09/2026). Análisis, recursos y tareas tipo.
// En el layout y no en cada página: cubre todos los `return` (vacío, error, contenido) y las rutas
// de abajo. No dibuja nada.
export default function BaseMaestraEnVivoLayout({ children }: { children: React.ReactNode }) {
  return (
    <>
      <RefrescarEnVivo tablas={TABLAS_DE.baseMaestra} />
      {children}
    </>
  )
}
