...
import { advertencias, claveDestino, destinos, itemPorDefecto, origenesDeItems } from '@/features/herramientas/logica/mover'
...
export default async function MoverCampo({ searchParams }: { searchParams: Promise<{ ids?: string; en?: string }> }) {
  ...
  // Los lotes salen del lugar donde está parado el teléfono (`en`), si tienen unidades ahí.
  const lugar = resolverLugar(p, lectura.obras, sp.en)
  const items = activos.map((a) => itemPorDefecto(p, a, lugar?.ubicacionId))
  const w = advertencias(p, activos, null, items)
  ...
  return (
    <MarcoTelefono titulo={titulo} volver={volverA}>
      {activos.length === 0 ? (
        ...
        <MoverTelefono
          items={items.map((it) => ({ id: it.activo.id, origen: it.origen, disponible: it.disponible }))}
          desde={origenesDeItems(p, items).map((g) => ({ rotulo: g.rotulo, cuenta: g.cuenta }))}
