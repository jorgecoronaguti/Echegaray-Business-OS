...
import { getMiObra, getMisTareas } from '@/features/empleado/services/empleadoService'
...
export default async function ReportarPage({
  ...
  if (!perfil.data?.persona_id) {
    return (
      <PantallaEmpleado titulo="Reportar problema" volver={{ href: '/mi-trabajo', label: 'Mi trabajo' }}>
  ...
  const [tareas, obras] = await Promise.all([getMisTareas(supabase), getMiObra(supabase)])
  ...
  return (
    <PantallaEmpleado titulo="Reportar problema" volver={{ href: '/mi-trabajo', label: 'Mi trabajo' }}>
      ...
      {obraId ? (
        <FormProblema tareas={tareas.data ?? []} obraId={obraId} tareaId={tarea ?? null} />
      ) : (
