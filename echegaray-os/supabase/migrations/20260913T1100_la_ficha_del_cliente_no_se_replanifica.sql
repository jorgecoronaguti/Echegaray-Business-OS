-- LA FICHA DEL CLIENTE DEJA DE PLANIFICARSE ENTERA EN CADA VISITA (13/09/2026)
--
-- El dueño: «no puedo acceder a cada cliente en el módulo Clientes, roto y no carga». Medido como
-- Dirección con la base tranquila: /clientes/messina 3,5 s; bajo carga pasaba el statement_timeout de
-- 8 s de `authenticator` y la pantalla no cargaba. La causa es la forma, no los datos:
-- `pantalla_cliente` es `LANGUAGE sql` con 21 claves y decenas de subconsultas, y una función SQL que
-- no se puede inlinear se vuelve a PLANIFICAR en cada llamada de una conexión nueva. Medido en la
-- misma sesión, mismo JSON (`=` sobre jsonb: true):
--
--     LANGUAGE sql      9.648 · 823 · 353 · 361 ms
--     LANGUAGE plpgsql  2.012 · 401 · 817 · 244 · 217 · 231 · 130 ms   (quattropani/cobranzas: 124 ms)
--
-- plpgsql guarda el plan por conexión y PostgREST reutiliza sus conexiones. El CUERPO NO CAMBIA NI UNA
-- LETRA: se copió de `pg_get_functiondef` de la función desplegada (no de una migración vieja) y va
-- dentro de `return (...)`. Verificado antes de aplicar con la huella md5 del JSON de cada cliente y
-- cada solapa contra la función anterior.
--
-- `create or replace` conserva los GRANT de la función. `stable` y el `search_path` se mantienen.
...
comment on function public.pantalla_cliente(text, text) is
  'LA FICHA DEL CLIENTE EN UN VIAJE. plpgsql desde 20260913T1100 para que el plan quede cacheado por '
  'conexión: el cuerpo es el mismo de 20260912T1300. p_solapa recorta el DIBUJO; quien recorta por rol '
  'es la RLS.';
