-- ═══ EL N DE LA SOLAPA DOCUMENTOS CUENTA LO QUE SE DIBUJA (dueño, 11/09/2026 17:50) ══════════════
--
-- «El crm dice documentos de drive (0) y está pésimo eso. Arreglar.»
--
-- Medido: `n_documentos` contaba `cliente_documento`, que son los vínculos hechos A MANO. San
-- Francisco tiene CERO de esos y 63 archivos en Drive; Messina tenía 113 contra 226 papeles reales.
-- El número de la solapa hablaba de otra cosa que las filas de abajo.
--
-- Ahora cuenta las TRES fuentes que la cara dibuja —los papeles de las obras del cliente, los
-- vínculos manuales y las órdenes del OS—, sin contar dos veces el mismo archivo. Es la ÚNICA
-- definición que se agrega, y no queda sola: `orquestador/lib/cara-documentos.pg.test.mjs` la compara
-- contra `armarCaraDocumentos()` —el TypeScript que dibuja— sobre los cinco clientes reales.
--
-- No cambia nada más de `pantalla_cliente`: mismo cuerpo de 20260911T2100 con esa clave reescrita.
...
comment on function public.pantalla_cliente(text, text) is
  'LAS DIECISIETE LECTURAS DE LA FICHA EN UN VIAJE. `papeles_obra` y `carpetas_obra` sólo en la cara '
  'Documentos. Desde 20260911T2200 `n_documentos` cuenta LO QUE LA CARA DIBUJA —papeles de obra + '
  'vínculos manuales + órdenes del OS, sin repetir un archivo— y no los vínculos manuales solos.';
