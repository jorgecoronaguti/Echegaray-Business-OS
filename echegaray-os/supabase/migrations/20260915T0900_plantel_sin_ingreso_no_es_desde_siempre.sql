-- SIN FECHA DE INGRESO NO ES «DESDE SIEMPRE» (15/09/2026).
--
-- Dueño, textual: «están mal las quincenas anteriores porque aparecen personas que son parte de un equipo de
-- subcontratistas». Diez personas dadas de alta el 01/09 «desde liquidación final del estudio» tienen
-- `personas.fecha_ingreso` NULL, y `activo` leía el NULL como «ya estaba»: el plantel de cualquier quincena vieja
-- las incluía. Un ingreso sin cargar no afirma nada. Regla, una sola definición con su espejo TS
-- (`plantelDeLaQuincena`, src/features/administracion/services/liquidacionPlantelActivo.ts) y JS
-- (`esDelPlantel`, orquestador/lib/costo-mo-quincena.mjs):
--
--   sin `fecha_ingreso`, una persona entra al plantel de la quincena SÓLO por actividad propia en ella (horas
--   pagas, recibo del período o línea de liquidación de la cerrada). Con la fecha cargada, rige la fecha.
--
-- En esta función `activo` sólo decide el camino del SUELDO MENSUAL sin actividad (`activo and t_nm_vig is not
-- null`): horas, recibo y línea ya eran caminos propios. Por eso el efecto medido en el costo puede ser cero; lo que
-- corrige es que la regla deje de contradecir a la de las pantallas.
--
-- ═══ CÓMO SE ESCRIBIÓ ═══
--
-- Desde la definición VIVA (`pg_get_functiondef`, 15/09/2026 09:30), que es la de 20260915T0850 byte a byte.
-- Único cambio: `p.fecha_ingreso is null or` → `p.fecha_ingreso is not null and`. `create or replace` conserva
-- owner y ACL; igual se repite la revocación de 20260915T0855 para que el archivo no dependa del orden.
...
-- 20260915T0855: el cálculo por persona no lo ejecuta authenticated (llega por `costo_mo_quincena`, que tiene su puerta).
