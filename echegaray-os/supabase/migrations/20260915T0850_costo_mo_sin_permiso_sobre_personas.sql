-- EL CRM ADMIN SE ROMPIÓ AL APLICAR 20260915T0800 (15/09/2026, dueño: «roto acceso a crm admin»).
--
-- `costo_mo_quincena_calculo` es SECURITY INVOKER y lee `personas.cuil` y `personas.convenio_colectivo`,
-- columnas SIN GRANT de SELECT para `authenticated` (a propósito: son datos laborales). Resultado:
-- `pantalla_clientes` y `pantalla_cliente` → «permission denied for table personas».
--
-- ARREGLO SIN ABRIR NINGÚN PERMISO: una función SECURITY DEFINER que devuelve sólo las columnas que el
-- cálculo necesita, y sólo a quien `es_administracion()` (dirección, administración, jefe de obra). A
-- cualquier otro rol le devuelve cero filas. No se toca GRANT ni RLS de `personas`.
...
$$;
revoke all on function public.persona_para_costo() from public, anon;
grant execute on function public.persona_para_costo() to authenticated, service_role;
