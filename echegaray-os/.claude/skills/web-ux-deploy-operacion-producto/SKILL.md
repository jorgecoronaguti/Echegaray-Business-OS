...
- **2026-07-09** — Primer deploy productivo completado: `https://echegaray-business-os.vercel.app`, verificado en vivo (`/login` responde 200). Proyecto importado desde GitHub con Root Directory `echegaray-os` y las 3 variables de entorno (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_SITE_URL`) en Production+Preview. Como el proyecto está conectado a GitHub, **cada `git push` a `main` dispara un redeploy automático** — no hace falta volver a tocar la UI de Vercel ni pedir autorización para cada redeploy futuro (regla ya actualizada arriba).
