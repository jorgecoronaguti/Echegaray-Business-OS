...
- **`cotizaciones_estado` / `registrar_cotizacion`** (`orquestador/lib/tools/cotizaciones-tool.mjs`) — la biblioteca viva de presupuestos y cotizaciones: qué presupuestos hay en borrador, embudo (en juego / ganadas / perdidas), tasa de conversión, monto cotizado y margen promedio. «¿Qué presupuestos tenemos en borrador?» se contesta de acá, sin modelo. Toda cotización nueva se registra acá para que deje aprendizaje.
