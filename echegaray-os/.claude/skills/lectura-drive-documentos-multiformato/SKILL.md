...
| Sheet muy grande (excede tokens) | Si `read_file_content` falla por tamaño, usar `download_file_content` con `exportMimeType` xlsx (base64), decodificar localmente y abrir con `openpyxl` (`data_only=True`) — **validado en este proyecto**: permite leer todas las pestañas reales de un Sheet de +1.000.000 de caracteres sin límite de tokens, iterando filas/columnas directamente en Python en vez de volcar el archivo entero al contexto. Leer solo las pestañas/rangos que responden la pregunta de negocio. |
...
## Gaps de conocimiento conocidos (primera versión, actualizado tras PR0-A)

- **Limitación de herramienta confirmada y ahora acotada con precisión**: `read_file_content` devuelve solo una pestaña por defecto. El método de `download_file_content` (exportando a `.xlsx`) + apertura local con `openpyxl` **sí está validado** para acceder a todas las pestañas reales de un archivo grande, sorteando el límite de tamaño del conector. Lo que **sigue sin resolverse** es el mapeo `gid` (de la URL) → pestaña: se probó leer `xl/workbook.xml` del `.xlsx` exportado, y el `sheetId` que Excel/OOXML asigna ahí (secuencial: 1, 2, 3...) **no coincide** con el `gid` real de Google Sheets — Google no preserva ese ID al exportar. Mitigación probada: en vez de intentar resolver el `gid`, leer todas las pestañas por nombre real (ya visibles tras la exportación) y elegir la relevante por contenido/nombre, dejando explícito que la elección fue por evidencia de contenido, no por `gid` confirmado.
