...
| la vista Caja de Analíticas (app.ecsas → Analíticas → Caja) | es la pestaña CAJA leída de su ESPEJO: `public.caja_sheet_foto` / vista `caja_sheet_vigente` (migración 20260918T1500) ← `orquestador/scripts/sync-caja-espejo.mjs` (sólo lectura del Sheet; núcleo puro por rótulo en `lib/caja-espejo.mjs`; lo dispara la sonda de 1 min y `echegaray-caja-espejo.timer` cada 10 min) · la app: `src/features/analiticas/components/VistaCaja.tsx` + `services/cajaSheet.ts` · lo gastado por FECHA DE CAJA sale de la vista `caja_egreso_percibido` | recalcular un saldo en la app; leer CAJA por letra de columna |
...
| pasar JORNALES (el Sheet) a `registros_hh` para la web (asistencia, ficha → Horas, plan vs real) | `orquestador/scripts/jornales-a-registros-hh.mjs` (`--dry` por defecto, `--aplicar`, `--anio 2025`) · la traducción pura y testeada en `lib/jornales-a-registros-hh.mjs` · `fuente_legacy = 'sheet:jornales'` · obra por `obra_alias` / nombre canónico del MISMO cliente; un rótulo que no resuelve es FALTA_DATO, **nunca la asignación sin fechas** · nunca borra; lo de otra fuente no se toca |
| historial de obra por persona (ficha → Asignaciones) reconstruido desde JORNALES | `orquestador/scripts/asignaciones-desde-jornales.mjs` (`--dry` por defecto, `--aplicar`, `--hoy`) · tramos y conciliación puros en `lib/asignaciones-desde-hh.mjs` · escribe `obra_asignacion` con `notas` marcadas `historial reconstruido desde JORNALES (sheet)` · nunca toca una fila web: recorta/cierra el tramo frente a una asignación vigente creada por una persona · idempotente por persona+obra+desde |
