...
## SESIÓN 2026-07-14 (tarde) — CANAL INTERACTIVO como capa de operación de la matriz
Se construyó la **capa de canal + capacidades** que hace usable el OS y arranca la Etapa 5 en Obras/Presupuestación. Todo server-side (extensión = cliente fino 0.8.0). Ver [[os-operacion-costo-extension]].
- **Canal confiable**: extensión↔motor interactivo (VM). Fix raíz de la inconfiabilidad = **créditos Anthropic agotados** (chat+worker misma key; auto-reload pendiente del dueño). Async robusto (no pierde tareas en reinicio → `lost`), progreso en vivo, versión visible.
- **Capacidades nuevas del cerebro (sin reinstalar)**: lee/interpreta **PDFs** del Drive (extracción local, 0 API); **multi-skill** (activa todas las capabilities que cruza el pedido, no una); **presupuesto guiado** paso a paso con **UOCRA Zona A jul-2026 verificado**; **F1 despacho a agentes durables** ("análisis en profundidad" → tarea al especialista real, ~128s); **list_obras** (índice de 31 obras = primer ladrillo de continuidad de dato de Obras, PRP-017 F0); "¿qué podés hacer?" y "¿cuánto gasté?" (0 API).
- **Ahorro de API** (pedido explícito del dueño: mínimo consumo + independencia + aprendizaje): prompt caching (system+tools), modelo por niveles (haiku simple/sonnet escribir·presupuestar), **vigilancia 4x/día → 1x/día**, telemetría de costo del chat.
- **Límite operativo clave**: la SA no crea/convierte archivos (sin storage) → "hacé un sheet" no puede; desbloqueo = Unidad Compartida o delegación de dominio (esta última además da Gmail/Calendar/Drive propio/multi-usuario). Scope Drive corregido a `drive` completo.
- PRPs escritos (no todos construidos): 016 (aprendizaje/independencia), 017 (obra+presupuestos), 018 (agentes/emergencia).

## PLAN DE COMPLETITUD (romper Etapas 5→6→7) — propuesto 2026-07-14
Principio: **sustrato horizontal una vez → activar por dominio en orden impacto×confianza** (ni 22 a ciegas, ni finanzas-solo). Techo A–C/D. Meta: madurez promedio 3,5 → 7+.
- **Bloque 0 — Desbloqueos del dueño (1 vez, habilitan todo)**: auto-reload créditos; Unidad Compartida (crear/convertir archivos); luego delegación de dominio (Gmail/Calendar/Drive propio/multi-usuario).
- **Bloque 1 — Sustrato horizontal (EL MURO, Etapa 5)**: (1) continuidad de dato por dominio (empezar Obras: PRP-017 F0→F2, costo real hoy en PDF a estructurar); (2) auto-acción Nivel D (detección→backlog/acción/seguimiento interno); (3) Decision/Outcome Ledger + aprendizaje (PRP-016 = la independencia que pidió el dueño); (4) trigger por evento/umbral además de cadencia.
- **Bloque 2 — Activación por dominio (impacto×confianza)**: Ola 1 (dato maduro N6-7): Obras, Presupuestación, Finanzas, Tesorería, Control Económico. Ola 2: Certificación, Adicionales, Clientes, Administración. Ola 3 (estructurar dato primero): Fiscal, Contabilidad, Laboral, Compras, Proveedores, Equipos, SySO, Comercial, Legal, Personas.
- **Bloque 3 — Etapa 6 (autónoma plena) + 7 (aprendizaje)**: watchers por obra/dominio (PRP-018 F2); el ledger cierra el lazo (cada obra mejora la próxima cotización). Emergencia de agentes desde el chat (PRP-018 F3).

## SESIÓN 2026-07-15 — PRP-016/017/018 CONSTRUIDOS Y VIVOS (bucle /loop autónomo)
Se construyó y verificó en vivo (motor interactivo en la VM) el grueso de los tres PRP. Todo son detecciones DETERMINÍSTICAS (0 API) o disparos internos Nivel D; el dato real gobierna (nunca fabricar). Ver [[cuadro-economico-obra]].
- **PRP-016 COMPLETO**: F1 captura correcciones del dueño (tool `aprender` + detección "recordá que…"), F2 re-inyecta lo aprendido, F3 propone revisar la skill del dominio ante RECURRENCIA ≥2 (`lib/skill-proposals.mjs` → `gap_skill` en backlog, no muta), F4 mide ("¿qué sabés de la empresa?" → `learnedSummary`).
- **PRP-017 núcleo**: F0 índice obras, F1 presupuesto guiado (UOCRA verificada + web_search precios), F2 **cuadro económico por obra** (`lib/obra-economics.mjs`: contratado↔presup↔costo real↔adicionales, margen/desvío, evidencia clasificada; tool `cuadro_economico_obra` para especialistas), F5 **cotización que aprende** (desvíos de obras cerradas inyectados al presupuestar). **F3 (adicionales) y F4 (avance físico) BLOQUEADOS POR DATO**: tabla `adicionales` vacía y Sheet "Avances de Obra" sin datos — necesitan carga del dueño, no se fabrica estructura.
- **PRP-018 COMPLETO**: F1 chat→especialista real, F2 watchers en la vigilancia autónoma (`desviosObras` + `alertasCaja` inyectan números YA CALCULADOS al Director), F3 **emergencia** (`lib/emergence.mjs`: gap del chat sin cobertura + modelo admite no poder → observación; recurrencia → propuesta de capacidad, no crea), F4 = PRP-016 F3.
- **Briefing ejecutivo** on-demand ("¿cómo estamos?" → caja vencida + desvíos de obra + backlog autónomo en una foto, 0 API).
- **Hallazgo testigo**: el OS tenía "margen de Galpones DESCONOCIDO" confirmado 5× → ahora lo calcula solo (cerró 19,9% vs 34,9% presupuestado, 23,2% sobre-costo). Gap convertido en hecho.
- **Módulos nuevos**: `obra-economics.mjs`, `caja-alertas.mjs`, `briefing.mjs`, `skill-proposals.mjs`, `emergence.mjs`, `tools/obra.mjs`. Detecciones nuevas en `interactive-server.mjs`. ~11 commits en `infra/anthropic-api-engine`.
- **Pendiente del dueño para desbloquear lo que falta**: cargar/estructurar adicionales y avance físico (o señalar la planilla que ya los tiene); auto-reload de créditos y delegación de dominio siguen pendientes.
