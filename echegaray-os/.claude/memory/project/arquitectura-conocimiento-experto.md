...
14 skills expertas en `.claude/skills/` (nivel `echegaray-os/`): 13 de dominio profesional de una empresa constructora (`ingenieria-civil-construccion`, `direccion-obra`, `planificacion-produccion`, `costos-presupuestacion`, `derecho-construccion-contratos`, `contabilidad-constructoras`, `impuestos-construccion`, `finanzas-tesoreria-construccion`, `derecho-laboral-construccion` (incluye UOCRA/IERIC/Fondo de Cese), `seguridad-higiene-art`, `calidad-obra`, `compras-abastecimiento-subcontratacion`, `gestion-empresarial-riesgos`) + 1 técnica agregada después (`integraciones-apis-sistemas-externos`, 2026-07-07): arquitectura de integraciones (APIs/webhooks/sincronización/reconciliación/migración de Sheets legacy). Es la única de las 14 que no es "profesional de constructora" — decide el *cómo* técnico de conectar el OS con un sistema externo, nunca el *qué* dato capturar (eso lo sigue decidiendo la skill de dominio dueña del dato: `finanzas-tesoreria-construccion`, `impuestos-construccion`, etc.). Ninguna integración real está construida todavía — parte de cero, sin inventar historial.
