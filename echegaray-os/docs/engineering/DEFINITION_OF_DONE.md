...
| XSAS · capacidad nativa de Drive (`orquestador/lib/drive/`, rama `xsas/drive-capability`) | — | **NO CIERRA** · auditado dos veces (31/08/2026); los dos bloqueos de la segunda vuelta corregidos, **sin re-auditar** | **3** — la migración `orq.drive_audit` está escrita, ensayada con rollback y **NO aplicada**, así que toda mutación devuelve `audit.registrado:false` y el criterio AUDIT está **incumplido** · `operation_execute.mjs` sigue marcando `executed` con lo que devolvió la tool, no con el efecto releído · la identidad no sobrevive a un rename/move (`drive_index` no reconcilia) y `file_id ↔ business_entity` no existe (9 tablas sin forma común) |
| XSAS · motor de planillas (`orquestador/lib/motores/planilla/`, rama `xsas/motor-planilla`) | — | **NO CIERRA** · auditado (31/08/2026); los 4 bloqueos corregidos, **sin re-auditar** | **2** — **la corrida contra Google NO se hizo** (freno de mano puesto): la promesa central «toda escritura relee y compara» está probada contra un doble escrito por la misma lane, y quedan sin evidencia el cálculo de la fórmula localizada, el aterrizaje de un `append`, la copia de fórmulas y el round-trip del rango con nombre · **el motor no tiene un solo consumidor**: `lib/tools/drive-write.mjs` conserva su lógica propia y la inversión no está aplicada |
| XSAS · motores de documento y presentación (`orquestador/lib/motores/`, rama `xsas/C-documentos`) | — | **NO CIERRA** · auditado (31/08/2026); los 5 bloqueos corregidos, **sin re-auditar** | **3** — **las lecturas no pasan por el portero**: `leerDocumento`/`exportarDocumento` no exigen actor ni respetan `archivos_habilitados` · los motores no están en `TOOLS_AUTORIZADAS_A_ESCRIBIR` (falta la firma del dueño), así que por la puerta de XSAS toda escritura da `PERMISSION_REQUIRED` · `origen: 'COPIA_DE_DRIVE'` está en el contrato y no implementado, y `planilla_control` devuelve `UNSUPPORTED_OPERATION` |
