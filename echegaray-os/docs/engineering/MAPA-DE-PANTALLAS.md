...
| **Cliente** · portal | `/portal` | `/portal`, `/portal/{documentos,facturas,pagos,terminadas}`, `/portal/documentos/<id>` y `/portal/recibo/<id>` sólo si son SUYOS. Entra por `/portal/login` con su mail (decisión del dueño 26/08, ratificada 25/09: la URL y la entrada del portal no cambian). `/portal/ingresar?t=` es un atajo opcional que Administración puede copiar desde la ficha |
